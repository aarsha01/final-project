...
# print(np.random.choice(imgdict[idxtocls[np.argmax(train_Y[0])]] ,replace=True))
	train_path = []
	for X, Y in zip(train_X, train_Y):
		train_path.append(np.random.choice(imgdict[idxtocls[np.argmax(train_Y[0])]]))

	test_path = []
	for X, Y in zip(test_X, test_Y):
		test_path.append(np.random.choice(imgdict[idxtocls[np.argmax(Y)]]))
