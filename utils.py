...
def preprocess_data(X, Y, P, resolution=128):
	X = tf.squeeze(X, axis=-1) # combine all 32 waypoints into single arr [[[val][val][val]......(32)]....(14)] ----> [[val,val,val.... (32)]....(14)]
	max_val = tf.reduce_max(X)/2.0 # finds tha max in the whole X
	X = (X - max_val) / max_val # normalize X , elemnts become of range (-1, 1)
	X = tf.transpose(X, [1, 0]) # transpose the matrix to [[val, val, val..... (14 channels)]......(32 waypoints)]
...
def load_complete_data(X, Y, P, batch_size=16, dataset_type='train'):	
	print(tf.data.Dataset.from_tensor_slices((X, Y, P)) )
